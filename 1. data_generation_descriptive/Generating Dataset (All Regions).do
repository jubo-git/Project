...
**# Diagnostic before defining variables 
	...
	**Total (standerdise first as tiers are intensity)
		egen z_tier_a = std(tierdays_a)
		egen z_tier_b = std(tierdays_b)
		egen z_retail = std(total_retail)

	* Combine the standardized units into an index, then quintile it
		gen total_index = z_tier_a + z_tier_b + z_retail
		xtile final_quintile = total_index, nquantiles(5)
