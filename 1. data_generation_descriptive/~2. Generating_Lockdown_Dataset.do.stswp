...
** Tidying (generic and making variables more streamlined)
	...
	* create a date function (to allow for averages later)
	...
	preserve
	keep if date == td(31dec2020) //*Duplicate Dec 31 rows 5 additional times 
	/// (creates 6 identical rows total per LA)
	expand 6
...
** COMBINING LOCAL AUTHORITY DATA
	...
	**Note this creates duplications that need to be resolved 
...
//Reveals that social distancing, primary, secondary hospitality had very little national variation. Just retail shutdown, tier a and tier b. Not using tiers as a weighting factor as it disguses national trends. Not possible or neccessary to build a total score.
	...
	* Per Tier Daily Count (BINARY)
		...
		gen total_days = days_tier1 + days_tier2 + days_tier3 + days_tier4
		
	**Intermediate collapse to get rid of duplicates 
	collapse (max) total_days tier days_tier1 days_tier2 days_tier3 days_tier4, by(onscode date)
	...
	** Collapse using to capture the total cumulative duration x severity
	collapse (sum) total_tier1 = days_tier1 ///
			 ...
			 (sum) total_tier = tier ///
			 (sum) total_tier_days = total_days, by(onscode)
